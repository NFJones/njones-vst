...
tresult PLUGIN_API Eris::initialize(FUnknown* context) {
    ...
    auto* time_window_param = new GainParameter(ParameterInfo::kCanAutomate, kTimeWindowId);
...
tresult PLUGIN_API Eris::process(ProcessData& data) {
    ...
    if (data.symbolicSampleSize == kSample32)
        auto notes = convert((Sample32**)in, numChannels, data.numSamples);
    else
        auto notes = convert((Sample64**)in, numChannels, data.numSamples);
    ...
    if (data.inputs[0].silenceFlags != 0) {
        ...
        for (int32 i = 0; i < numChannels; i++) {
            // do not need to be cleared if the buffers are the same (in this case input buffer are already cleared by
            // the host)
            if (in[i] != out[i]) {
    ...
    if (bBypass) {
        ...
        if (gain < 0.0000001) {
            ...
            if (data.symbolicSampleSize == kSample32)
                fVuPPM = processAudio<Sample32>((Sample32**)in, (Sample32**)out, numChannels, data.numSamples, gain);
            else
                fVuPPM = processAudio<Sample64>((Sample64**)in, (Sample64**)out, numChannels, data.numSamples, gain);
        }
...
tresult PLUGIN_API Eris::setState(IBStream* state) {
    ...
    double saved_time_window = 0.0;
    if (streamer.readDouble(saved_time_window) == false)
        return kResultFalse;
...
tresult PLUGIN_API Eris::getState(IBStream* state) {
    ...
    streamer.writeDouble(time_window);
